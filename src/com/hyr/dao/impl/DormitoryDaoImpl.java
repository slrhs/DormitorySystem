...
public class DormitoryDaoImpl implements DormitoryDao {
    ...
    public Integer availableId(List<Integer> dormitoryIdList) {
        Connection connection = JDBCUtil.getConnection();
        String dormitoryIdStr=dormitoryIdList.stream().map(String::valueOf).collect(Collectors.joining(","));
        String sql = "select id from dormitory where available > 0 and id not in("+dormitoryIdStr+")  limit 0,1 ";
        PreparedStatement statement = null;
        ResultSet resultSet = null;
        Integer result = null;
        try {
